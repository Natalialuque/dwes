...
function validaEntero(int &$var, int $min, int $max, int $defecto): bool{
  // Usamos filter_var para validar como entero
    $filtrado = filter_var($var, FILTER_VALIDATE_INT, [
    ...
    // Si no pasa el filtro, devolvemos el valor por defecto
    $var = $defecto;
    return false;
}
...
/**
 * unction validaReal(float &$var, float $min, float $max, float $defecto):bool Esta función 
...
function validaReal(float &$var, float $min, float $max, float $defecto): bool{
   //usamos filtrado para comprobar que es real 
    $filtrado = filter_var($var, FILTER_VALIDATE_FLOAT, [
        'options' => ['min_range' => $min, 'max_range' => $max]
    ]);
...
*@return boolean
 */
//NO exite FILTER_VALIDATE_DATE en filter_var, por eso lo hacemos a mano
