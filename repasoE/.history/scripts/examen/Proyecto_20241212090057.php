...
class Poryecto
{
     ...
     *
     */
    public function __construct(string $nombre, string $empresa, int $fechainicio = 1, string $fechaInicio ="", string $fechaFin="", int $duracion, int $tipo )
    {


        if (!validaCadena($nombre, 40, $nombre) || trim($nombre) == "") {

            throw new Exception("La propiedad nombre es obligatoria y tiene que tener 40 car");
            
        }
        else $this->_nombre = $nombre;

        if (!validaCadena($empresa, 40, $empresa) || trim($empresa) == "") {

            throw new Exception("La propiedad empresa es obligatoria");
