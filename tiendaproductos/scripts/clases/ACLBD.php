...
class ACLBD extends ACLBase{
    ...
    public function getCodRole(string $nombre):int|false
    {
        ...
        $sentencia = "SELECT cod_acl_role FROM acl_roles WHERE nombre = '{$this->_sqli->real_escape_string($nombre)}'";
        $resul = $this->_sqli->query($sentencia)->fetch_assoc();
    ...
    function getPermisoRole(int $codRole, int $numero): bool{
        ...
        $campo = "perm" . $numero;
        $consulta = "SELECT $campo FROM acl_roles WHERE cod_acl_role = $codRole";
         $resul = $this->_sqli->query($consulta)->fetch_assoc();

         if (is_null($resul))
             return false;

        return boolval($resul[$campo]);
    }
    ...
    function existeCodUsuario(int $codUsuario): bool
    {
        if (!$this->_hayConeccion)
          ...
          $consulta = "SELECT cod_acl_usuario FROM acl_usuarios WHERE cod_acl_usuario = $codUsuario";
          $resul = $this->_sqli->query($consulta)->fetch_assoc();
   ...
   function esValido(string $nick, string $contrasena): bool
    {
        ...
        $consulta = "SELECT * FROM acl_usuarios WHERE nick = '$nick' AND contrasenia = '$contrasena'";
        $resul = $this->_sqli->query($consulta)->fetch_assoc();
    ...
    function setUsuarioRole(int $codUsuario, int $role): bool
    {
        ...
        return $this->_sqli->query($consulta) === true;
    }
    ...
    function dameRoles(): array
    {
        ...
        $consulta = "SELECT cod_acl_role, nombre FROM acl_roles ORDER BY cod_acl_role";
        $datos = $this->_sqli->query($consulta);
        $res = [];
